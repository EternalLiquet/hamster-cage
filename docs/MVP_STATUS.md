...
| #16 Offices | Integration refresh [#60](https://github.com/EternalLiquet/hamster-cage/pull/60) | Local office list/add/edit and registration-intent projection preserve saved facts. Three focused API37 UI cases pass, including restored optimistic version and precise fractional radius. After #17 permission integration, required build/lint/unit/security and Android-test compilation pass; renewed independent gates remain pending. Actual registration/future capture stop requires #18; keep issue open until evidenced. |
...
| #18 Geofence capture | Implementation | [Capture contract](GEOFENCE_CAPTURE.md): non-exported receiver, enabled-office registration, immutable receipt/observation facts, replay and sanitized failures. Required build/lint/unit/security pass; four deterministic API37 cases and opt-in Play Services registration/removal probe pass. Real closed-UI transition delivery and independent gates remain pending. Reboot/coverage boundary is #19, security audit #20. |
...
| #22 Dashboard | Implementation | Engine-backed totals, honest current state, visible-only refresh and process-restart checks in progress. |
