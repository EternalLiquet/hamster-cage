...
Implementation checks: `:core-domain:test :app:testDebugUnitTest :app:lintDebug :app:assembleDebug` and `python scripts/security_check.py` pass. Five instrumented storage tests passed on the local Pixel 10 Pro XL emulator (API 37): all-fact restart and aggregate recomputation, atomic failed batch, version-one schema compatibility, legacy policy migration with future-version rejection, unsupported Room version and corrupt-file preservation. Review then identified delayed observations after an office is disabled; the repository now retains these raw facts for existing offices. A fresh DataStore reopen, delayed-observation restart and visible-failure regression await device execution. Independent verification/review, current-head CI and PR merge remain pending. No version-two schema exists; any future upgrade requires an explicit, preservation-tested migration.
