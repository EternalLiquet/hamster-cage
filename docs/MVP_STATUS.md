...
| #26 Explanations | Review [#67](https://github.com/EternalLiquet/hamster-cage/pull/67) | Eight pure traceability/DST cases, lint and two focused API37 explanation navigation/200%-text/raw-evidence checks pass. Hosted failures were exact-text test matcher errors and inherited history observer, now corrected. Refreshed on merged #25; final gates pending. [Contract](DAY_EXPLANATIONS.md). |
| #27 Corrections | Implementation | Preview/confirm editor, explicit manual provenance and append-only revert with Room migration 1→2 implemented. Full domain suite plus two input cases, lint and five focused API37 repository/UI/schema-1 migration checks pass, including preview parity, reopen, revert and failed/stale writes. Final independent gates pending. [Contract](CORRECTIONS.md). |
