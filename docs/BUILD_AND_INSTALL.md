...
The [CI workflow](../.github/workflows/android.yml) checks out the exact PR head or master commit, tests it, builds the debug APK, verifies its signature, and publishes the APK with those metadata files. `scripts/artifact_provenance.py` refuses a dirty checkout, an unexpected variant, mismatched checksum or missing signing digest. The completed #34 evidence identifies the earlier version-code-2 hosted artifact; it does not identify the #77 version-code-3 build. The local pre-PR phone checkpoint below has its own source, APK hash and signing identity, and the final #77 hosted artifact is still pending. Never rely on a PR number alone to identify APK bytes.
