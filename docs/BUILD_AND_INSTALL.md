...
## Current local candidate (scoped #35 security pass)

The final local candidate is an exact debug build of #97 source `0b12b2f6ea779d3ed39dc77d8f68c30226f8b692`, merged on master as `48b4fb7216e9d8bcfc189e5f5fd948a68968d860`. It is `dev.hamstercage.preview`, version code 3, `minSdk` 26 and `targetSdk` 36. Its APK SHA-256 is `1d885c3c7f2b80efcc99500c4c0c915292b3f6e38d419cd715841439bf5dcd98`; local Android debug signing-certificate SHA-256 is `2af9f1b4d765f6fc10727071b7570d7b225f47026e514f935322778c8517ead1`. The local ignored `.delivery/product-mvp-preview/BUILD.txt` accompanies these APK bytes. This development key is not a durable production update key. A hosted CI artifact may use a different key even at the same source, so compare its own `BUILD.txt`, checksum and signer before installing.

The [#35 phone receipt](https://github.com/EternalLiquet/hamster-cage/issues/35#issuecomment-5827146852) records `adb install -r` and a cold MainActivity launch of this exact candidate on a Samsung API36 phone. It verifies install/launch, not a complete office or capture journey.
...
3. With synthetic facts, check that Dashboard Today and History open the same chronological Day Detail, correction uses local date/time pickers and honest DST prompts, credited intervals remain explainable, calendar exclusions/WFH and policy settings survive a process restart, and unknown coverage stays explicit. Avoid real coordinates in screenshots or logs.
4. Check the first-run/Settings uninstall and clear-storage warning; on a same-key update, confirm synthetic offices/policy facts remain. After a separately confirmed history delete, confirm attendance/calendar facts disappear while offices/base policy remain; full reset intentionally clears all app data.

The earlier 2026-09-24 pre-PR phone checkpoint (`86aa7d3`) verified version-code-3 install/launch over version 2 only. The current #97 candidate and its same-key update are identified above. The hosted #77 APK has a different development signer and cannot update that installed local-key package in place. Do not uninstall a populated preview to force an update: no export/restore path exists.

Emulator install/update smoke and phone install/launch support packaging and local persistence only. **Phone office setup, UI-closed multi-office geofence delivery, weekday reconciliation timing, permission/reboot/force-stop recovery, battery/OEM behavior, small-device accessibility, and manufacturer backup/device-transfer behavior remain unverified.** These are separate open acceptance gates; this debug APK does not establish production release security.
