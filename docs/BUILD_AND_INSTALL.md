...
Clearing storage in Android Settings is a separate destructive operation, not an update or restart. The in-app **Delete attendance and calendar history** and **Reset all app data** controls show different scope previews and require confirmation; the latter requests Android to clear the whole app. Review [local-data behavior](PRIVACY_CONTROLS.md) before using either control.
