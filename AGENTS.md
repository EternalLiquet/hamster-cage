...
- One reusable agent per stage role works across issues: a distinct implementer, independent verifier, and independent reviewer/integrator. The implementer writes/tests and opens the PR. The verifier independently reads the issue, inspects the code and runs checks, recording PASS/FAIL, exact commit and unverified criteria. The reviewer independently checks correctness, maintainability, security/privacy, scope and verification evidence.
- Where agent model selection is supported, use GPT-6 Sol Medium for coordination, implementation, verification and documentation, and Sol High for independent review. Do not silently substitute Astra or a higher effort setting; tell the owner when coordinator model selection must be made manually. Model choice never collapses the three independent stage roles or the sole merger.
- Give each stage a compact handoff: issue and acceptance criteria, relevant files and diff, exact source SHA, existing evidence and remaining gaps. Run focused development checks first; reuse trustworthy exact-head CI while the verifier independently establishes behavior. Assign one CI watcher, avoid redundant polling, and release idle agent slots for ready work.
...
- A dedicated documentation agent reconciles the final Phase 0–5 public Markdown and private authoritative project sources in focused issue #98 after the code/security merges. The coordinator keeps epic/issue checklists tied to accepted evidence. Keep every status claim tied to its focused PR and independently verified result; leave unverified criteria open. Never copy private-only assets, personal data or private repository metadata into public evidence.
...
The local install preview is a debug build with a separate `.preview` package; its exact source, APK checksum and signer must accompany each delivery. A same-key install/launch is not office-flow or geofence acceptance. Production signing and real-device background/battery validation remain explicit gates. Public release hardening is post-MVP, but critical correctness/privacy defects block MVP. Each work checkpoint records evidence on its focused issue/PR. The final focused documentation PR for issue #98 reconciles `docs/MVP_STATUS.md` and the corresponding private project sources after code/security integration.
